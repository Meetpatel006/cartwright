...
export class WixAdapter implements PlatformAdapter {
  public readonly name: PlatformType = "wix";
  ...
  public detect(win?: Window, doc?: Document): boolean {
    ...
    const hasGlobals = Boolean(
      (w as unknown as { wixData?: unknown })?.wixData ||
        (w as unknown as { wixStores?: unknown })?.wixStores,
    );
    const hasDom = Boolean(
      d?.querySelector('meta[name="generator"][content*="Wix"]') ||
        d?.querySelector('link[href*="static.wixstatic.com"]') ||
        d?.querySelector("wix-dropdown-menu"),
    );
    ...
    this.clickListener = (e: Event) => {
      tryCatchGuard(() => {
        ...
        const btn = target.closest(
          '[data-hook="add-to-cart"], [data-hook="product-add-to-cart"], button[data-hook="add-to-cart"]',
        );
        if (!btn) return;
      ...
      }, undefined, this.context?.debug, "WixAdapter:addToCartClick");
    };
  ...
  public extractProduct(doc?: Document): CanonicalProduct | null {
    ...
    return tryCatchGuard(() => {
      const titleEl = d.querySelector('[data-hook="product-title"], h1');
      const title = titleEl?.textContent?.trim() || "";
      if (!title) return null;

      const priceEl = d.querySelector('[data-hook="formatted-primary-price"], [data-hook="product-price"]');
      const parsed = priceEl?.textContent ? parsePriceText(priceEl.textContent) : {};

      const skuEl = d.querySelector('[data-hook="sku"]');
  ...
  public extractCart(doc?: Document): CanonicalCart | null {
    ...
    const countEl = d.querySelector('[data-hook="cart-icon-count"]');
  ...
  public extractOrder(doc?: Document): CanonicalOrder | null {
    ...
    return tryCatchGuard(() => {
      const orderEl = d.querySelector('[data-hook="order-number"]');
