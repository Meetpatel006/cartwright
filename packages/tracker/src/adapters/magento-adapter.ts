/**
 * Magento / Adobe Commerce Storefront Adapter
 */
...
export class MagentoAdapter implements PlatformAdapter {
  public readonly name: PlatformType = "magento";
  ...
  public detect(win?: Window, doc?: Document): boolean {
    ...
    const hasGlobals = Boolean((w as unknown as { Mage?: unknown })?.Mage);
    const hasDom = Boolean(
      d?.body?.classList.contains("catalog-product-view") ||
        d?.querySelector('[data-role="priceBox"]') ||
        d?.querySelector('script[src*="mage/"]'),
    );
  ...
  public init(emitter: EventEmitter, context: AdapterContext): void {
    ...
    this.clickListener = (e: Event) => {
      tryCatchGuard(() => {
        ...
        const btn = target.closest("#product-addtocart-button, .action.tocart, [data-action='add-to-cart']");
        if (!btn) return;
      ...
      }, undefined, this.context?.debug, "MagentoAdapter:addToCartClick");
    };
  ...
  public extractProduct(doc?: Document): CanonicalProduct | null {
    ...
    return tryCatchGuard(() => {
      const titleEl = d.querySelector(".page-title .base, .product-info-main .page-title");
      const title = titleEl?.textContent?.trim() || "";
      if (!title) return null;

      const priceEl = d.querySelector('[data-price-type="finalPrice"] .price, .price-box .price');
      const parsed = priceEl?.textContent ? parsePriceText(priceEl.textContent) : {};

      const skuEl = d.querySelector('[itemprop="sku"], .product.attribute.sku .value');
  ...
  public extractCart(doc?: Document): CanonicalCart | null {
    ...
    const countEl = d.querySelector(".counter-number, .showcart .counter.qty");
  ...
  public extractOrder(doc?: Document): CanonicalOrder | null {
    ...
    return tryCatchGuard(() => {
      const orderEl = d.querySelector(".checkout-success .order-number strong, .order-number");
